...
//StreamAPI
//Java8 도입되었음, 컬렉선(List,set)과 같이 배열에 저장된 데이터를 선언형으로 처리하기 위한
//추상화된 반복 프레임워크
//필터링,매핑,정렬,집계 등등
...
public class StringApi {
    public static void main(String[] args) {
        ...
        List<String> result = englishNames.stream()
                ...
                //()-> , :: 같은 작용
                //하지만() -> 는 각각요소를 매개변수로 받아서 매개변소를 참조하여 메소드를 호출
        ...
        System.out.println("이름의 길이가 3이상인것(일반 for문) : "+count + "개");

        long count1 = englishNames.stream()
                .filter(name->name.length() >= 4)
                .count(); //long으로 반환
        System.out.println("이름의 길이가 3이상인것(stream) : "+count1 + "개");
